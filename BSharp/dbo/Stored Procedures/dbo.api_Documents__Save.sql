...
DECLARE @DocumentsLocal [dbo].[DocumentList] , --@LinesLocal [dbo].[LineList], 
		@EntriesLocal [dbo].[EntryList];
DECLARE @DocumentsLocalResultJson NVARCHAR(MAX), --@LinesLocalResultJson NVARCHAR(MAX), 
		@EntriesLocalResultJson NVARCHAR(MAX);
	-- Fill in the blanks
	...
	INSERT INTO @DocumentsLocal SELECT * FROM @Documents --dbo.[fw_Documents__Json](@DocumentsLocalResultJson);
--	INSERT INTO @LinesLocal SELECT * FROM dbo.[fw_Lines__Json](@LinesLocalResultJson);
	INSERT INTO @EntriesLocal SELECT * FROM @Entries-- dbo.[fw_Entries__Json](@EntriesLocalResultJson);
