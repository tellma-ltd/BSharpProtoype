...
BEGIN
	...
	BEGIN TRANSACTION
		BEGIN TRY
			...
			DELETE FROM dbo.Resources WHERE Id IN (SELECT Id FROM @Resources WHERE Status = N'Deleted');
			...
			(
				...
				USING (
					SELECT [Id], [ResourceType], [Name], [Code], [UnitOfMeasure], [Memo], [Lookup1], [Lookup2], [Lookup3], [Lookup4], [GoodForServiceParentId], [FungibleParentId]
					FROM @Resources 
				...
				WHEN NOT MATCHED THEN
					INSERT ([ResourceType],		[Name],		 [Code], [UnitOfMeasure],	 [Memo],	[Lookup1],	[Lookup2],	[Lookup3],		[Lookup4], [GoodForServiceParentId], [FungibleParentId])
					VALUES (s.[ResourceType], s.[Name], s.[Code], s.[UnitOfMeasure], s.[Memo], s.[Lookup1], s.[Lookup2], s.[Lookup3], s.[Lookup4], s.[GoodForServiceParentId], s.[FungibleParentId])
