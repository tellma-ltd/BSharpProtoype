...
AS
DECLARE @Basic int = (SELECT [Id] FROM dbo.Resources WHERE [Name] = N'Basic');
DECLARE @Transportation int = (SELECT [Id] FROM dbo.Resources WHERE [Name] = N'Transportation');
SELECT
	A.TaxIdentificationNumber As [Employee TIN],
	C.[Name] As [Employee Full Name],
	SUM(CASE 
		WHEN (S.AccountId = N'ShorttermEmployeeBenefitsAccruals' AND S.ResourceId = @Basic)
			THEN S.Direction * S.[Value] Else 0 
		END) AS [Basic Salary],
	SUM(CASE
		WHEN (S.AccountId = N'ShorttermEmployeeBenefitsAccruals' AND S.ResourceId = @Transportation)
			THEN S.Direction * S.[Value] Else 0 
		END) AS [Overtime],
	SUM(CASE 
