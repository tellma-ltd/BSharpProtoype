...
BEGIN TRY
	...
	WHILE @i < 30
	BEGIN
		SELECT * FROM [dbo].[fn_AssetRegister](@fromDate, @toDate);
		SELECT @fromDate = DATEADD(DAY, 90, @fromDate), @toDate = DATEADD(DAY, 90, @toDate);
		SET @i = @i + 1;
	END
		SELECT * FROM dbo.[fn_AssetRegister]('2017.02.01', '2018.02.01');
	...
	SELECT * FROM dbo.ft_Paysheet(default, default, '2018.02');
	ROLLBACK;
END TRY
BEGIN CATCH
	ROLLBACK;
